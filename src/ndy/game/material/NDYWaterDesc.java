...
public class NDYWaterDesc {
	...
	public void calcConst() {
		for(int i=0;i<amplitude.length;i++) {
			wconst[i*4] = FloatMath.sqrt(NDYMath.TWOPI * NDYGame.instance.mWeather.mGravity / wavelength[i]);
	...
	public void getWaveNormal(float x, float y, NDYVector3 N, int i) {
		N.x = 0.f;
		N.y = 1.f;
		N.z = 0.f;
	...
	public void getNormal(float x, float y, float t, NDYVector3 N) {
		N.x = 0.f;
		N.y = 1.f;
		N.z = 0.f;
		for(int i=0;i<amplitude.length;i++) {
