...
public class ProgramBasic extends Program {
	...
	public void sendSubmesh(SubMesh submesh) {
		...
		if (submesh.vbo != -1)
			Renderer.glVertexAttribPointer(mPositionHandle, 3, GLES20.GL_FLOAT, false, submesh.vsize, 0);
		else
		...
		if (submesh.getDesc() == SubMesh.VERTEX_DESC_POSITION_TEXCOORDS || submesh.getDesc() == SubMesh.VERTEX_DESC_POSITION_NORMAL_TEXCOORDS) {
			if (mTextureHandle != -1) {
				if (submesh.vbo != -1) {
					Renderer.glVertexAttribPointer(mTextureHandle, 2, GLES20.GL_FLOAT, false, submesh.vsize, submesh.texcoordsOffset);
				} else {
		...
		if (submesh.getDesc() == SubMesh.VERTEX_DESC_POSITION_NORMAL || submesh.getDesc() == SubMesh.VERTEX_DESC_POSITION_NORMAL_TEXCOORDS) {
			if (mNormalHandle != -1) {
				if (submesh.vbo != -1) {
					Renderer.glVertexAttribPointer(mNormalHandle, 3, GLES20.GL_FLOAT, false, submesh.vsize, submesh.normalOffset);
				} else {
		...
		if (submesh.getDesc() == SubMesh.VERTEX_DESC_POSITION_COLOR) {
			if (mColorHandle != -1) {
				if (submesh.vbo != 1) {
					Renderer.glVertexAttribPointer(mColorHandle, 4, GLES20.GL_FLOAT, false, submesh.vsize, submesh.colorOffset);
				} else {
