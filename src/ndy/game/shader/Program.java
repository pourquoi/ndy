...
public class Program extends Ressource {
	...
	public static Program factory(String programName) {
		Program p = (Program)Ressource.getRessource(programName);
		if( p == null ) {
			if(programName.contains("water")) {
				p = new ProgramWater(programName);
			} else {
				p = new ProgramBasic(programName);
			}
			Ressource.addRessource(p);
		}
		return p;
	}
